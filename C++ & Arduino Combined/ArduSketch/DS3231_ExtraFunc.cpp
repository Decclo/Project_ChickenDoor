/*
 * DS3231_ExtraFunc.cpp
 *
 * Created: 21-04-2017 19:11:42
...
#include <stdio.h>
#include <avr/io.h>
#include "DS3231_ExtraFunc.h"


void usage(void)
{
	PRINTS("\n[MD_DS3231_Tester by Marco Colli, modified by Decclo]");
	PRINTS("\n?\thelp - this message");
	...
	PRINTS("\n\ne\tExit Debugging Mode and return to normal operation");
}
...
char *sts2String(codeStatus_t code, boolean bHexValue)
{
	if (!bHexValue)
	{
		switch (code)
...
{
	PRINTS("\n>- Settings -<");
	PRINT("\n/EOSC Halt Enabled:\t", sts2String(RTC.status(DS3231_CLOCK_HALT), false));
	PRINT("\nBBSQW Sq Wave Enable:\t", sts2String(RTC.status(DS3231_SQW_ENABLE), false));
	PRINT("\nRS1_2 Sq Wave Freq:\t", sts2String(RTC.status(DS3231_SQW_TYPE), false));
	PRINT("\nCONV  Temp Convert:\t", sts2String(RTC.status(DS3231_TCONV), false));
	PRINT("\nINTCN Int enable:\t", sts2String(RTC.status(DS3231_INT_ENABLE), false));
	PRINT("\n      12h mode:\t\t", sts2String(RTC.status(DS3231_12H), false));
	PRINT("\nEN32k 32kHz enabled:\t", sts2String(RTC.status(DS3231_32KHZ_ENABLE), false));
	PRINT("\nAGING Aging Offset:\t", sts2String(RTC.status(DS3231_AGING_OFFSET), true));
	
	PRINTS("\n\n>- Status -<");
	PRINT("\nBSY  Busy Flag:\t\t", sts2String(RTC.status(DS3231_BUSY_FLAG), false));
	PRINT("\nOSF  Halted Flag:\t", sts2String(RTC.status(DS3231_HALTED_FLAG), false));
	#if ENABLE_TEMP_COMP
	...
	PRINTS("\n\n>- Alarm 1 at ");
	showAlarm1();
	PRINT("\nA1IE   Intrpt:\t", sts2String(RTC.status(DS3231_A1_INT_ENABLE), false));
	PRINT("\nA1F    Flag:\t", sts2String(RTC.status(DS3231_A1_FLAG), false));
	PRINT("\nA1M1_4 Type:\t", alm2String(RTC.getAlarm1Type()));

	PRINTS("\n\n>- Alarm 2 at ");
	showAlarm2();
	PRINT("\nA2IE   Intrpt:\t", sts2String(RTC.status(DS3231_A2_INT_ENABLE), false));
	PRINT("\nA2F    Flag:\t", sts2String(RTC.status(DS3231_A2_FLAG), false));
...
void doAlarm1()
{
	...
	switch (toupper(c))
	{
		...
		Serial.println("Enable interrupt? 1 for yes, 0 for no");
		...
		PRINT(" value ", sts2String(value, false));
		...
		case 'T': // alarm type
		Serial.println("Set type: 0=all s, 1=s, 2=s+m, 3=s+m+h, 4=s+m+h+dt, 5=s+m+h+dy");
		c = readNext();
		switch (toupper(c))
		{
		...
		Serial.println("What kind of alarm do you want?");
		Serial.println("0=all m, 1=m, 2=m+h, 3=m+h+dt, 4=m+h+dy");
		c = readNext();
		switch (toupper(c))
		{
			case '0': alm = DS3231_ALM_SEC;   break;
		...
		Serial.println("Please input time as yyyymmdd hhmmss dw");
		inputTime();
		RTC.writeAlarm1(alm);
		showAlarm1();
		break;
